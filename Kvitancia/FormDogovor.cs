...
namespace Kvitancia
{
    public partial class FormDogovor : Form
    {
        ...
        public void UpdateData()
        {
            ...
            lico.Load(new SqlCommand("select * from Natural_Person", db.GetConnection()).ExecuteReader());
            dogovor.Load(new SqlCommand("select * from FizLico", db.GetConnection()).ExecuteReader());
            obrzovat.Load(new SqlCommand("select * from Educational_Program", db.GetConnection()).ExecuteReader());
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            double stoimost = Convert.ToDouble(textBox3.Text);
            string srok = textBox4.Text;
            string datanah = textBox5.Text;
            string datakonh = textBox6.Text;

            db.OpenConnection();
            new SqlCommand($"insert into fizlico values('{fio}', '{obrazovanie}', {stoimost}, '{srok}', '{datanah}', '{datakonh}')", db.GetConnection()).ExecuteNonQuery();
            db.CloseConnection();
        ...
        {
            db.OpenConnection();
            pr.Load(new SqlCommand($"select * from Educational_Program where name = '{comboBox2.SelectedItem.ToString()}'", db.GetConnection()).ExecuteReader());
